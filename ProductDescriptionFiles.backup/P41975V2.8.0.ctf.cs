...
namespace NDC8.Business
{
    ...
    public class P41975V2_8_0: ComponentHandlerBase4
    {
        ...
        public override void Init()
        {
            ...
            {                
                ...
                bool bEnable = false;
                if ((HasComponent("LaserNavigator") ||
                                             ...
                                             string oldVCT, string newVCT)
        {
            ...
            // Make LS5 not being mandatory since we can from now on add and remove dynamically      
            List<ComponentDataBase> LS5Devices = 
                    VehicleApplication2.GetComponentsByType(ContractNodeType.LaserScanner);
            foreach(var currentLS5Device in LS5Devices)
            {
                if(currentLS5Device.Type == "LaserScanner")
                {
                    foreach(var currentDevice in devices)
                    {
                        if(currentDevice.DeviceName == currentLS5Device.DeviceName)
                        {
                            currentDevice.Mandatory = false;
                        }
        ...
        public override void AddLaserScanner(string connectionPointName, ContractNodeType componentType)
        {
            try
            {
                ...
                device = VehicleApplication2.CreateDevice("LaserScanner", ContractNodeType.LaserScanner, "CVC600", "COM2", false);
                device.AddReference("Port", "COM2");

                ComponentDataBase laserNavigator = VehicleApplication2.GetComponentByName("LaserNavigator");
                laserNavigator.AddReference("Scanner", device.DeviceName);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Script error", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
            ...
            try
            {
                ...
                switch (radiotype)
                {
                    case ContractNodeType.Rcu:
                        device = VehicleApplication.CreateDevice("Rcu", ContractNodeType.Rcu, "CVC600", comPortName, false);
                    ...
                    case ContractNodeType.Satel:
                        device = VehicleApplication.CreateDevice("Satel", ContractNodeType.Satel, "CVC600", comPortName, false);
                        device.AddReference("Port", comPortName);
                    ...
                    case ContractNodeType.IRModem:
                        device = VehicleApplication.CreateDevice("IRModem", ContractNodeType.IRModem, "CVC600", comPortName, false);
                        device.AddReference("Port", comPortName);
        ...
        public override bool OkToAddDevice(ComponentDataBase Parentdevice, ContractNodeType dataType)
        {
            ...
            switch (dataType)
            {
                ...
                case ContractNodeType.SickDevice:
                    if (!(HasComponent("WallNavigator") || HasComponent("NaturalNavigator")))
                    {
                        return false;
                    }
                    else
                    {
                        int numberOfSickSensors = VehicleApplication2.GetNumberOfComponents(ContractNodeType.SickDevice);
                        return numberOfSickSensors < 2;
                    }
                case ContractNodeType.LaserScanner:
                    if (!(HasComponent("LaserNavigator") || HasComponent("ReflectorInit")))
                    {
                        return false;
                    }
                    else
                    {
                        int numberOfLaserScanners = VehicleApplication2.GetNumberOfComponents(ContractNodeType.LaserScanner);
                        return numberOfLaserScanners < 2;
                    }
        ...
        {
            ComponentData mcd = VehicleApplication2.CreateDevice("Mcd", "MCD7P", "CVC600", "COM3", false);
        ...
        public void addLS2000(ScriptNode node)
        {
            try
            {
                ...
                {
                    ComponentData device = VehicleApplication2.CreateDevice(nodeTemplate.DeviceName, "LS2000Scanner", "CVC600", node.ParentName, false);
                    if (HasComponent("LaserNavigator"))
                    {
                        ComponentDataBase laserNavigator = VehicleApplication2.GetComponentByName("LaserNavigator");
                        laserNavigator.AddReference("Scanner", device.DeviceName);
        ...
        public void addLanComPort(ScriptNode node)
        {
            try
            {
                ...
                ComponentData comp = VehicleApplication2.CreateDevice(node.DeviceName, "LanSerialPort", "CVC600", node.ParentName, false);
                ComponentDataBase Lan = VehicleApplication.GetComponentByName(node.ParentName);
